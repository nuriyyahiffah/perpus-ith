...
<body class="antialiased" x-data="{ profileOpen: false }">
        ...
        <div class="container mx-auto flex justify-between items-center px-6">
            <div class="flex items-center space-x-4">
                ...
                <div class="border-l border-white/20 pl-4">
                    <span class="text-[11px] font-black leading-none uppercase tracking-tighter block">
                        Perpustakaan<br><span class="text-yellow-400">Institut Teknologi Bacharuddin Jusuf Habibie</span>
                    </span>
                </div>
            </div>
            ...
            <div class="flex items-center space-x-6">
                ...
                <div class="relative group">
                    <a href="{{ route('notifikasi.index') }}" class="text-white hover:text-yellow-400 transition-all duration-300 relative">
                        ...
                        @if($unreadCount > 0)
                            <span class="absolute -top-2 -right-2 bg-rose-500 text-white text-[9px] font-black w-5 h-5 flex items-center justify-center rounded-full ring-2 ring-[#1E293B] animate-bounce">
                                {{ $unreadCount }}
                            </span>
                        @endif
                    </a>
                ...
                <div class="relative">
                    <button @click="profileOpen = !profileOpen" class="flex items-center space-x-4 focus:outline-none group">
                        ...
                        <div class="relative">
                            ...
                            <span class="absolute -bottom-1 -right-1 block h-2.5 w-2.5 rounded-full bg-emerald-500 ring-2 ring-[#1E293B]"></span>
                        </div>
                    </button>
                    ...
                    <div x-show="profileOpen" @click.away="profileOpen = false" x-transition x-cloak
                        ...
                        <a href="{{ route('profil.edit') }}" class="flex items-center gap-4 px-5 py-3 hover:bg-slate-50 hover:text-amber-600 transition-colors group">
                            <div class="w-8 h-8 rounded-lg bg-amber-50 flex items-center justify-center text-amber-600 group-hover:bg-amber-600 group-hover:text-white transition-all">
                                <i class="bi bi-person-gear"></i>
                            </div>
                            <span class="text-[11px] font-extrabold uppercase">Edit Profil</span>
                        </a>

                        <a href="{{ route('mahasiswa.riwayat') }}" class="flex items-center gap-4 px-5 py-3 hover:bg-slate-50 hover:text-emerald-600 transition-colors group">
                            <div class="w-8 h-8 rounded-lg bg-emerald-50 flex items-center justify-center text-emerald-600 group-hover:bg-emerald-600 group-hover:text-white transition-all">
                                <i class="bi bi-clock-history"></i>
                            </div>
                            <span class="text-[11px] font-extrabold uppercase">Riwayat Pinjam</span>
                        </a>

                        <a href="{{ route('reservasi.index') }}" class="flex items-center gap-4 px-5 py-3 hover:bg-slate-50 hover:text-indigo-600 transition-colors group border-t border-slate-50">
                            <div class="w-8 h-8 rounded-lg bg-indigo-50 flex items-center justify-center text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white transition-all">
                        ...
                        <div class="border-t border-slate-100 mt-2 pt-2">
                            <form action="{{ route('logout') }}" method="POST">
                                @csrf
                                <button type="submit" class="w-full flex items-center gap-4 px-5 py-3 text-rose-500 hover:bg-rose-50 font-bold text-left transition-all group">
                                    <div class="w-8 h-8 rounded-lg bg-rose-50 flex items-center justify-center text-rose-600 group-hover:bg-rose-500 group-hover:text-white transition-all">
    ...
    {{-- HERO SECTION --}}
    <header class="hero-gradient pt-12 pb-24 relative overflow-hidden">
        <div class="container mx-auto flex flex-col md:flex-row items-center px-6">
            <div class="md:w-1/2 z-10 text-center md:text-left">
                ...
                <p class="text-slate-600 text-sm max-w-sm leading-relaxed italic mx-auto md:mx-0">
                    Temukan referensi terbaik untuk mendukung perkuliahanmu di Prodi <strong>{{ Auth::user()->prodi }}</strong>.
                </p>
    ...
    <main class="container mx-auto -mt-10 px-6 pb-20 relative z-20">
        ...
        {{-- SECTION 1: REKOMENDASI PRODI --}}
        <section class="mb-16">
            <div class="mb-8">
                <h2 class="text-xl font-black text-[#2D3E50] uppercase tracking-tighter leading-none">Rekomendasi Prodi {{ Auth::user()->prodi }}</h2>
                <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2">Buku pilihan dosen untuk prodi anda</p>
            </div>
                ...
                <div class="group">
                    <div class="bg-white rounded-[2.5rem] p-6 shadow-xl shadow-slate-200/50 border border-slate-50 flex flex-col transition-all duration-300 hover:-translate-y-2 h-full">
                        <div class="aspect-[3/4] rounded-[2rem] bg-slate-100 mb-5 overflow-hidden relative shadow-inner">
                            ...
                            {{-- Badge Stok Habis Kecil di Pojok --}}
                            @if((int)$item->buku->stok <= 0)
                                <div class="absolute top-3 right-3">
                                    <span class="bg-rose-500/90 backdrop-blur-sm text-white text-[8px] font-black uppercase px-2 py-1 rounded-lg shadow-lg border border-white/20">Habis</span>
                                </div>
                            @endif
                        </div>

                        <div class="flex-grow">
                            <h3 class="font-black text-[#2D3E50] text-sm uppercase leading-tight mb-1 line-clamp-2">{{ $item->buku->judul }}</h3>
                            <p class="text-[10px] text-slate-400 font-bold uppercase tracking-tighter mb-4">By: {{ $item->buku->penulis }}</p>
                        </div>

                        <div class="mt-auto pt-4 border-t border-slate-50">
                            {{-- Selalu arahkan ke Detail agar user bisa baca deskripsi --}}
                            <a href="{{ route('buku.detail', $item->buku->id) }}" class="w-full py-3 bg-[#2D3E50] hover:bg-indigo-600 text-white text-[10px] font-bold uppercase rounded-xl flex items-center justify-center transition-all shadow-lg">
                                <i class="bi bi-eye me-2"></i> Detail & Pinjam
                            </a>
                        </div>
                    </div>
                </div>
        ...
        <section class="mb-16">
            <div class="mb-8">
                ...
                <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2">Buku yang paling sering dipinjam</p>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                ...
                <div class="group">
                    <div class="bg-white rounded-[2.5rem] p-6 shadow-xl shadow-slate-200/50 border border-slate-50 flex flex-col transition-all duration-300 hover:-translate-y-2 h-full">
                        <div class="aspect-[3/4] rounded-[2rem] bg-slate-100 mb-5 overflow-hidden relative shadow-inner">
                            ...
                            {{-- Label Populer --}}
                            <div class="absolute top-3 left-3 bg-indigo-600 text-white text-[7px] font-black uppercase px-2 py-1 rounded-md shadow-lg">
                        ...
                        <div class="flex-grow">
                            <h3 class="font-black text-[#2D3E50] text-sm uppercase leading-tight mb-1 line-clamp-2">{{ $buku->judul }}</h3>
                            <p class="text-[10px] text-slate-400 font-bold uppercase tracking-tighter mb-4">By: {{ $buku->penulis }}</p>
                        </div>

                        <div class="mt-auto pt-4 border-t border-slate-50">
                            <a href="{{ route('buku.detail', $buku->id) }}" class="w-full py-3 bg-rose-500 hover:bg-rose-600 text-white text-[10px] font-bold uppercase rounded-xl flex items-center justify-center transition-all shadow-lg">
                                <i class="bi bi-eye me-2"></i> Detail & Pinjam
                            </a>
                        </div>
                    </div>
                ...
                @empty
                <div class="col-span-full py-16 text-center bg-slate-50 rounded-[2rem] border border-dashed border-slate-200">
                    <p class="text-slate-400 font-bold uppercase text-[9px] tracking-widest">Belum ada data peminjaman populer.</p>
                </div>
                @endforelse
            </div>
        </section>
        ...
        <section>
            <div class="mb-8">
                <h2 class="text-xl font-black text-[#2D3E50] uppercase tracking-tighter leading-none">Koleksi Terbaru</h2>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                ...
                <div class="group">
                    <div class="bg-white rounded-[2rem] p-5 shadow-lg shadow-slate-100 border border-white flex flex-col transition-all duration-300 hover:shadow-xl h-full">
                        <div class="aspect-[3/4] rounded-2xl bg-slate-100 mb-4 overflow-hidden relative">
                            <img src="{{ $buku->gambar_buku ? asset('images/' . $buku->gambar_buku) : asset('images/default-cover.jpg') }}" class="w-full h-full object-cover">

                            {{-- Badge Stok Habis Kecil --}}
                            @if((int)$buku->stok <= 0)
                                <div class="absolute top-2 right-2">
                                    <span class="bg-rose-500 text-white text-[7px] font-black uppercase px-2 py-1 rounded shadow-lg">Habis</span>
                                </div>
                            @endif
                        </div>

                        <h3 class="font-bold text-[#2D3E50] text-[12px] uppercase leading-tight line-clamp-2 mb-2">{{ $buku->judul }}</h3>
                        ...
                        <div class="mt-auto pt-4 flex justify-between items-center border-t border-slate-50">
                            ...
                            <a href="{{ route('buku.detail', $buku->id) }}" class="text-indigo-600 hover:translate-x-1 transition-transform" title="Detail Buku">
                ...
                <div class="col-span-full text-center py-20 bg-white rounded-[3rem] border-2 border-dashed border-slate-200">
                    <p class="text-slate-400 text-xs font-bold uppercase tracking-widest mb-4">Belum ada koleksi buku terbaru.</p>
                </div>
                @endforelse
            </div>
        </section>
