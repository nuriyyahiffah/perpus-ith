<aside class="w-72 bg-[#10192A] text-white hidden lg:flex flex-col shrink-0 sticky top-0 h-screen sidebar-scroll font-medium transition-none">
    ...
    <nav class="flex-1 px-6 space-y-2 pb-10 overflow-y-auto custom-scrollbar">
        ...
        <div x-data="{ open: {{ request()->is('admin/mahasiswa*') || request()->is('admin/dosen*') || request()->is('admin/pustakawan*') || request()->is('shared/pegawai*') ? 'true' : 'false' }} }">
            <button @click="open = !open" 
                class="w-full flex items-center justify-between px-7 py-4 rounded-2xl transition-colors duration-200 {{ request()->is('admin/mahasiswa*') || request()->is('admin/dosen*') || request()->is('admin/pustakawan*') || request()->is('shared/pegawai*') ? 'bg-yellow-400 text-[#10192A]' : 'hover:bg-slate-800/50 text-slate-300 hover:text-white' }}">
